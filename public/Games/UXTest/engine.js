...
(async function init() {
    ...
    try {
        ...
        // Clean up previous script tags to prevent memory leaks when switching rooms
        ...
        scripts.forEach(script => {
            if (script.data?.code) {
                ...
                // IIFE removed. Scripts now execute in the global scope so objects can see them.
                globalCode += '\n/* --- ' + script.label + ' --- */\n' + pureCode + '\n';
            }
        });
        ...
        window.KinemeEngine = {
            _isSwitching: false,
            switchRoom: function(identifier, trans = "none") {
                if (this._isSwitching) return; // Prevent spam firing
                ...
                if (trans === "none") {
                    ...
                    bootRoom();
                    this._isSwitching = false; // Unlock
                    return;
                }
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        this._isSwitching = false; // Unlock after transition finishes
                    }, 500);
        ...
        const gameLoop = (time) => {
            ...
            // Only run logic on layers that are currently visible/active
